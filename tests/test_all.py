...
#   assert result["conv"])
#   .assertEqual(np.sum(result["result"]), sum(m0))
...
#   .assertEqual(np.sum(p["result"]), p["pop"])
...
#   assert p["conv"])
#   # check overall population and marginals correct
#   .assertAlmostEqual(np.sum(p["result"]), p["pop"]) # default is 7d.p.
...
#   assert np.allclose(np.sum(p["result"], (1)), m0))
#   assert p["conv"])
#   .assertEqual(p["iterations"], 1)
#   .assertEqual(p["maxError"], 0.0)
#   .assertEqual(p["pop"], 100.0)
#   assert np.array_equal(p["result"], np.array([[45.24, 6.76], [41.76, 6.24]])))
...
#   assert np.allclose(np.sum(p["result"], (1)), m0))
#   assert p["conv"])
#   .assertLess(p["iterations"], 6)
#   .assertLess(p["maxError"], 5e-10)
#   .assertEqual(p["pop"], 100.0)
...
#   #print(p)
#   assert p["conv"])
#   .assertLess(p["chiSq"], 0.04)
#   .assertGreater(p["pValue"], 0.9)
...
#   assert p["conv"])
#   .assertLess(p["chiSq"], 73.0) # TODO seems a bit high (probably )
#   .assertGreater(p["pValue"], 0.0) # TODO this is suspect
#   .assertEqual(p["pop"], 100.0)
#   assert np.allclose(np.sum(p["result"], (0, 1)), m2))
#   assert np.allclose(np.sum(p["result"], (1, 2)), m0))
#   assert np.allclose(np.sum(p["result"], (2, 0)), m1))
...
#   assert p["conv"])
#   .assertLess(p["chiSq"], 10)
#   .assertGreater(p["pValue"], 0.002) # TODO this looks suspect too
#   .assertEqual(p["pop"], 100)
...
#   assert p["conv"])
#   .assertLess(p["chiSq"], 10)
#   .assertGreater(p["pValue"], 0.27)
#   .assertEqual(p["pop"], 120)
...
#   assert p["conv"])
#   .assertLess(p["chiSq"], 70) # seems a bit high
#   .assertGreater(p["pValue"], 0.0) # seems a bit low
#   .assertEqual(p["pop"], 100.0)
#   assert np.allclose(np.sum(p["result"], (0, 1)), m2))
#   assert np.allclose(np.sum(p["result"], (1, 2)), m0))
#   assert np.allclose(np.sum(p["result"], (2, 0)), m1))
...
#   assert p["conv"])
#   .assertLess(p["chiSq"], 5.5)
#   .assertGreater(p["pValue"], 0.02)
#   .assertEqual(p["pop"], 100)
#   assert np.allclose(np.sum(p["result"], (0, 1, 2)), m3))
#   assert np.allclose(np.sum(p["result"], (1, 2, 3)), m0))
#   assert np.allclose(np.sum(p["result"], (2, 3, 0)), m1))
...
#   .assertEqual(p, "seed dimensions [2, 3, 7, 5] are inconsistent with that implied by marginals ([2, 3, 5, 7])")
...
#   s = np.ones((2,3,5))
#   p = hl.qisi(s, [np.array([0,1]), np.array([1,2]), np.array([2,3])], [m1, m2, m3])
#   .assertEqual(p, "seed dimensions 3 is inconsistent with that implied by marginals (4)")
