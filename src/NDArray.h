...
public:
  ...
  explicit NDArray(const std::vector<int64_t>& sizes)
      : m_dim(sizes.size()), m_sizes(sizes), m_storageSize(0), m_data(0), m_owned(true) {
    resize(sizes);
  }

  // Construct with storage managed by some other object
  NDArray(const std::vector<int64_t>& sizes, T* const storage) : m_dim(sizes.size()), m_sizes(sizes) {
    assert(m_sizes.size());
  ...
  // Copying is strongly discouraged for efficiency reasons, however there will always be times when a copy is
  // unavoidable... By explictly providing a copy function we avoid sloppy/inefficient coding where implicit copies are
  // (inadvertently) taken
  ...
  const std::vector<int64_t>& sizes() const { return m_sizes; }

  size_t storageSize() const { return m_storageSize; }

  const T* rawData() const { return m_data; }
  ...
  void assign(T val) const { std::fill(m_data, m_data + m_storageSize, val); }
...
// STL-like debugging
  ...
  value_type* begin() const { return m_data; }

  value_type* end() const { return m_data + m_storageSize; }
...
private:
  ...
  T* allocate(size_t size) const { return new T[size]; }

  void deallocate(T* p) const { delete[] p; }
