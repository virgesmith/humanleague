...
public:
  ...
  IPF(const typename Microsynthesis<double, M>::index_list_t& indices,
      typename Microsynthesis<double, M>::marginal_list_t& marginals)
  ...
  const std::vector<NDArray<double>>& errors() const { return m_errors; }

  double maxError() const { return m_maxError; }

  bool conv() const { return m_conv; }
