...
NB In the notation below, the index $i$ is scalar and refers to a particular dimension. The index $k$ is a vector index of length equal to the dimensionality of the problem.
...
$$\sum\limits_{k\neq i} (\mathbf{T_i})_k = \mathbf{m}_i$$
...
$$\mathbf{r}_i = \sum\limits_{k\neq{i}} \mathbf{T}_k - \mathbf{m}_i$$ (5)
...
$$\mathbf{T}_{k\neq i} \leftarrow \mathbf{T}_{k\neq i} - \mathbf{r}_i$$ (6)

Repeat this step for each dimension. This now ensures that constraint (1) is met (N.B. (3) is already met), but may have resulted in contraint (4) being violated.
...
4. If the seed population does not satisfy constraint (3) we apply the adjustments detailed above. In order not to violate constraint (4) the vector in dimension $i$ is chosen such that 

$$\min\limits_i \mathbf{T}_k \ge \max\limits_i\Big(\sum\limits_{k\neq{i}} \mathbf{T}_{k} - \mathbf{m}_i\Big)$$ (7)
...
of being in state $(i,j)$. We take 100 (i.e. the population) random samples from this distribution to create a seed population $\mathbf{T}$ (in bold), which for example may look like:

 $\mathbf{m}_0\setminus\mathbf{m}_1$ |  35    | 40     | 25     | 
:-------:|--------|--------|--------|----
    51   | **19** | **20** | **11** | *50*
    49   | **17** | **20** | **13** | *50*
    ~    |  *36*  |  *40*  |  *24*  |
