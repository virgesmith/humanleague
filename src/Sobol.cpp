...
Sobol::Sobol(size_t dim, uint32_t nSkip)
    : m_dim(dim), m_buf(dim), m_pos(dim) // ensures m_buf gets populated on 1st access
{
  if (dim < 1 || dim > 1111) {
...
Sobol::~Sobol() { nlopt_sobol_destroy(m_s); }
...
void Sobol::skip(uint32_t n) { nlopt_sobol_skip(m_s, n, m_buf.data()); }
...
uint32_t Sobol::dim() const { return m_dim; }

uint32_t Sobol::min() const { return 0; }

uint32_t Sobol::max() const { return std::numeric_limits<uint32_t>::max(); }
