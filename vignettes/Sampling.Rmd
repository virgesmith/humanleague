...
There have even been a number of software packages aimed at facilitating IPF for spatial microsimulation and other applications [].

However, the method has various limitations from the perspective of spatial microsimulation for social simulation, most IPF's generation of fractional weights instead of integer populations [@lovelace_truncate_2013], and its handling of 'empty cells' [@lovelace_evaluating_2015].
These are well-documented, as illustrated in an outstanding PhD thesis by @zaloznik_iterative_2011, and papers documenting developments aimed at overcoming barriers reducing the utility of IPF for spatial microsimulation.



Nor is it able to generate states containing exactly zero population. IPF requires an initial starting "seed" population (often simply setting each state to a population of one) and updates the matrix iteratively, stopping when the marginals are matched to withing some specified tolerance.
