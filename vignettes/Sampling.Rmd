...
The algorithms described in this paper are outlined in this sectio.
...
Table 2: Computational efficiency of the algorithms in terms of execution time.
...
<!-- Name the algorithms by name in full again here at outset -->
The algorithms presented here have substantial advantages over traditional IPF, primarily the generation of integer populations by default.
Furthermore, we have demonstrated that integerising fractional populations generated by other methods may result in populations that no longer match the marginals. 

In terms of performance, IQRS and IWRS algorithms are faster than the IPF implementation we compared against (`mipfp`). This is likely due to the fact the algorithms are not iterative in nature, and that the implementation is in a low-level language. The performance of population synthesis is unlikely to be the bottleneck in any workflow, so performance is not considered a major advantage.
