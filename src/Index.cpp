...
Index::operator const std::vector<int64_t>&() const { return m_idx; }

size_t Index::size() const { return m_idx.size(); }

const std::vector<int64_t>& Index::sizes() const { return m_sizes; }

// allow read-only access to individual values
const int64_t& Index::operator[](size_t i) const { return m_idx[i]; }

// allow modification of individual values
int64_t& Index::operator[](size_t i) { return m_idx[i]; }
...
bool Index::end() const { return m_atEnd; }

TransposedIndex::TransposedIndex(const std::vector<int64_t>& sizes) : Index(sizes) {}
...
MappedIndex::operator const std::vector<int64_t*>&() const { return m_mappedIndex; }

// allow read-only access to individual values
const int64_t& MappedIndex::operator[](size_t i) const { return *m_mappedIndex[i]; }

// allow modification of individual values
int64_t& MappedIndex::operator[](size_t i) { return *m_mappedIndex[i]; }

bool MappedIndex::end() { return m_atEnd; }

FixedIndex::FixedIndex(const std::vector<int64_t>& sizes, const std::vector<std::pair<int64_t, int64_t>>& fixed)
    : m_freeDim(sizes.size() - fixed.size()), m_fullIndex(sizes), m_freeSizes(sizes.size() - fixed.size()),
      m_atEnd(false) {
...
//
FixedIndex::operator const Index&() const { return m_fullIndex; }

// allow read-only access to individual values
const int64_t& FixedIndex::operator[](size_t i) const { return *m_freeIndex[i]; }

// allow modification of individual values
int64_t& FixedIndex::operator[](size_t i) { return *m_freeIndex[i]; }

bool FixedIndex::end() { return m_atEnd; }

const std::vector<int64_t*>& FixedIndex::free() const { return m_freeIndex; }

const std::vector<int64_t>& FixedIndex::sizes() const { return m_freeSizes; }
