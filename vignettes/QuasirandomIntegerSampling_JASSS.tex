...
handles `empty cells' poorly \cite{lovelace_evaluating_2015}; and requires a
...
date is a Ph.D. thesis on the subject (\cite{zaloznik_iterative_2011}).
...
Consider the following simple example: we wish to calculate the expected value of the sum of 2 fair and independent dice, sampling 100 times. Using pseudorandom sampling, with a standard error proportional to the square root of the number of samples, we would expect to get an estimate of \(7 \pm 10\%\). However, using quasirandom sampling, since the standard error is inversely proportional to the number of samples, the estimate will be \(7 \pm 1\%\). To achieve the same accuracy with peudorandom numbers would require 10000 samples.
...
implementations of the Sobol sequence generator (\cite{johnson_stevengj/nlopt:_nodate}) and the
incomplete gamma function (\cite{burkardt_asa032_2008}\cite{noauthor_notitle_nodate}), the latter in order to 
compute the p-value.
...
\url{https://github.com/CatchDat/humanleague}. It has been submitted for inclusion in the official CRAN repository of \texttt{R} packages.
