...
template <typename T, typename U> void diff(const NDArray<T>& x, const NDArray<U>& y, NDArray<double>& d) {
...
template <typename T> T sum(const std::vector<T>& v) { return std::accumulate(v.begin(), v.end(), T(0)); }

template <typename T> T product(const std::vector<T>& v) {
  return std::accumulate(v.begin(), v.end(), T(1), std::multiplies<T>());
}

template <typename T> T sum(const NDArray<T>& a) {
  return std::accumulate(a.rawData(), a.rawData() + a.storageSize(), T(0));
}

template <typename T> T min(const NDArray<T>& a) {
...
template <typename T> void print(const std::vector<T>& v, std::ostream& ostr = std::cout) {
...
template <typename T> void print(T* p, size_t n, size_t breakAt = 1000000, std::ostream& ostr = std::cout) {
...
template <typename T> bool isZero(const std::vector<T>& v) {
  for (size_t i = 0; i < v.size(); ++i) {
    if (v[i] != 0)
      return false;
  }
...
template <typename T> T marginalProduct(const std::vector<std::vector<T>>& m, const std::vector<int64_t>& idx) {
...
template <typename T> std::vector<T> reduce(const NDArray<T>& input, size_t orient) {
  // return reduced;
  // check valid orientation
  if (!(orient < input.dim()))
    throw std::runtime_error("reduce dimension " + std::to_string(orient) + " greater than array dimension " +
...
template <typename T> NDArray<T> reduce(const NDArray<T>& input, const std::vector<int64_t>& preservedDims) {
...
template <typename T> NDArray<T> slice(const NDArray<T>& input, std::pair<int64_t, int64_t> index) {
...
template <typename T> std::vector<std::vector<int>> listify(const size_t pop, const NDArray<T>& t, int offset = 0) {
...
template <> inline bool allclose(const std::vector<double>& a, const std::vector<double>& b, double abstol) {
  if (a.size() != b.size()) {
    return false;
  }
