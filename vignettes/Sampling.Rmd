...
Given a discrete distribution of frequencies $\{f_0,f_1,...f_n\}$, after each random sample $i \in \{0...n\}$ with probability 

$$p(i) = \frac{f_i}{\sum\limits_{j=0}^{n}f_j}$$

we adjust the distribution to $\{f_0,f_1,...f_i-1,....f_n\}$. Once $f_i$ has reached zero not further samples can take the value $i$ since $p(i) = 0$. Sampling continues until $f_i = 0 \space\forall\space i \in \{0...n\}$.
...
We consider a 2 dimensional case here for simplicity, for a population of 100 individuals with two categorical characteristics. Let the marginal vectors be $\mathbf{m}_0=\{51,49\}$ and $\mathbf{m}_1=\{35,40,25\}$, giving the population 6 possible states.    
...
We compare the three algorithms using a number of test cases.

No integerisation was done to the IPF data (using e.g. the `R` package `rakeR`) due to that fact that integerisation only guarantees constraints (1) and (3) are met, and can in fact break constraint (2) on a non-integer input that meets contraints (1) and (2). The $\chi^2$ value resulting from IPF is thus likely to be slightly lower due to the fact the numbers need not be integral.

IPF was initialised with a random seed array, uniformly distributed in $[0,2P/S)$

The first three tests are made-up examples. Input data for the remaining tests are marginal figures derived from 2011 UK census data at the MSOA level (mid-layer super output area), one dimension representing person status in terms of ags,sex, and economic activity, the other dimension their workplace location.


 Dim | Pop | States | N | $\bar{c}$ | IQRS $\bar{p}$  | $\bar{t}$ | $\bar{c}$ | IWRS $\bar{p}$ | $\bar{t}$ | $\bar{c}$ | IIPF $\bar{p}$ | $\bar{t}$ 
:-:|---|---|---|--------|--------|--------|--------|--------|--------|-------|-------|------
...
We measure the performance of the algorithms using the same tests as above.
...
The table ean execution time in milliseconds of the three algorithms for the 2-dimensional test cases
